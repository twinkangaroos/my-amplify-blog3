...
export default function MShopUpdateForm(props) {
  const {
    id: idProp,
    mShop,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = mShopRecord
      ? { ...initialValues, ...mShopRecord }
      : initialValues;
    setShop_name(cleanValues.shop_name);
    setArea(cleanValues.area);
    setPrefecture(cleanValues.prefecture);
    setSort(cleanValues.sort);
    setErrors({});
  };
  const [mShopRecord, setMShopRecord] = React.useState(mShop);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp ? await DataStore.query(MShop, idProp) : mShop;
      setMShopRecord(record);
    };
    queryData();
  }, [idProp, mShop]);
  React.useEffect(resetStateValues, [mShopRecord]);
  const validations = {
    shop_name: [],
    area: [],
    prefecture: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            MShop.copyOf(mShopRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "MShopUpdateForm")}
      {...rest}
    >
      <TextField
        label="Shop name"
