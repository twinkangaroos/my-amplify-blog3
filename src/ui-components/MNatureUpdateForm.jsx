...
export default function MNatureUpdateForm(props) {
  const {
    id: idProp,
    mNature,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = mNatureRecord
      ? { ...initialValues, ...mNatureRecord }
      : initialValues;
    setNature(cleanValues.nature);
    setNature_name(cleanValues.nature_name);
    setSort(cleanValues.sort);
    setErrors({});
  };
  const [mNatureRecord, setMNatureRecord] = React.useState(mNature);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp ? await DataStore.query(MNature, idProp) : mNature;
      setMNatureRecord(record);
    };
    queryData();
  }, [idProp, mNature]);
  React.useEffect(resetStateValues, [mNatureRecord]);
  const validations = {
    nature: [],
    nature_name: [],
    sort: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            MNature.copyOf(mNatureRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "MNatureUpdateForm")}
      {...rest}
    >
      <SelectField
        label="Nature"
