...
export default function MTypeHairUpdateForm(props) {
  const {
    id: idProp,
    mTypeHair,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = mTypeHairRecord
      ? { ...initialValues, ...mTypeHairRecord }
      : initialValues;
    setType_hair(cleanValues.type_hair);
    setType_hair_name(cleanValues.type_hair_name);
    setSort(cleanValues.sort);
    setErrors({});
  };
  const [mTypeHairRecord, setMTypeHairRecord] = React.useState(mTypeHair);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(MTypeHair, idProp)
        : mTypeHair;
      setMTypeHairRecord(record);
    };
    queryData();
  }, [idProp, mTypeHair]);
  React.useEffect(resetStateValues, [mTypeHairRecord]);
  const validations = {
    type_hair: [],
    type_hair_name: [],
    sort: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            MTypeHair.copyOf(mTypeHairRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "MTypeHairUpdateForm")}
      {...rest}
    >
      <SelectField
        label="Type hair"
